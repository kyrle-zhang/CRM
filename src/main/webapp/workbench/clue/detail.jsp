...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveRelationBtn").click(function (){
			//先找出所有被选中的市场活动
			$select = $("input[name=selectOne]:checked");
			if($select.length == 0){
				alert("请选择要关联的市场活动");
			}else {
				let parameter = "clueId=${clue.id}&";
				for(let i=0;i<$select.length;i++){
					parameter += "activityId=";
					parameter += $select[i].value;
        ...
        $("#select-Activity").keydown(function (event){
            ...
            if(event.keyCode==13){
                ...
                $.ajax({
                    ...
                    success : function (data){
                        ...
                        $.each(data,function (i,n){
                            html += '<tr>';
                            html += '<td><input name="selectOne" type="checkbox" value="'+ n.id +'"/></td>';
                            html += '<td>'+ n.name +'</td>';
                ...
                //bootStrap中，在模态窗口中按下回车键会默认关闭模态窗口
                //可以在这里打断默认命令的执行
                return false;
            }
        })
	...
	/**
	 * 删除线索和市场活动之间的关系
	 * @param id 线索和市场活动关系表的id
	 */
	function unBundle(id){
		$.ajax({
			url : "workbench/clue/deleteBundleById.do",
			data : {
				"id" : id,
			},
			dataType : "json",
...
<body>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" id="relateActivityBtn" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
